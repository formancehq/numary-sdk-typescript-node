...
export class AccountsApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async listAccounts(ledger: string, pageSize?: number, after?: string, address?: string, metadata?: any, balance?: number, balanceOperator?: 'gte' | 'lte' | 'gt' | 'lt' | 'e' | 'ne', paginationToken?: string, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;
        ...
        if (balanceOperator !== undefined) {
            requestContext.setQueryParam("balance_operator", ObjectSerializer.serialize(balanceOperator, "'gte' | 'lte' | 'gt' | 'lt' | 'e' | 'ne'", ""));
        }
